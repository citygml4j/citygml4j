...
public class UriAttributeAdapter extends AbstractGenericAttributeAdapter<UriAttribute> {
    ...
    @Override
    public UriAttribute createObject(JsonNode node, Object parent) throws CityJSONBuildException {
    ...
    @Override
    public void writeObject(UriAttribute object, ObjectNode node, CityJSONSerializerHelper helper) throws CityJSONSerializeException, CityJSONWriteException {
        if (object.getName() != null && object.isSetValue()) {
            node.put(object.getName(), object.getValue());
            registerType(object.getName(), GenericAttributeType.URI_ATTRIBUTE, helper);
        }
    }
}
