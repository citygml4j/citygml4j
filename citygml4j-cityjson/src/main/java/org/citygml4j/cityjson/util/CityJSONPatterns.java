...
public class CityJSONPatterns {
    ...
    public static final Pattern CRS_URL = Pattern.compile("^https?://www.opengis.net/def/crs/(.+?)/(.*?)/([0-9]+)$");
    public static final Pattern CRS_URN = Pattern.compile("^urn:ogc:def:crs(?:,crs)?:(.+?):(.*?):([0-9]+).*");
    public static final Pattern CRS_SHORT = Pattern.compile("^(.+?):([0-9]+)$");
}
