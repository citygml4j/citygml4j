...
public class OpeningPropertyImpl extends AssociationImpl<Opening> implements OpeningProperty {
		...
		if (openingPropertyType.isSet_Object()) {
			...
			if (opening.getValue() != null) {
				...
				if (elem instanceof DoorType)
					return ModelMapper.BLDG_1.toCityGML((DoorType)elem, name);
				else if (elem instanceof WindowType)
					return ModelMapper.BLDG_1.toCityGML((WindowType)elem, name);
				else if (elem instanceof AbstractOpeningType)
					return ModelMapper.BLDG_1.toCityGML((AbstractOpeningType)elem, name);
			}
		}
	...
	public void setObject(Opening object) {
		...
		if (object instanceof Window)
			jaxbElem = ModelMapper.BLDG_1.toJAXB((Window)object);
		else if (object instanceof Door)
			jaxbElem = ModelMapper.BLDG_1.toJAXB((Door)object);
		else if (object instanceof Opening)
			jaxbElem = ModelMapper.BLDG_1.toJAXB((Opening)object);
