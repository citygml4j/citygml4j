...
public class Relief100ModelMapperImpl extends AbstractCityGMLModelMapperImpl {
	...
	public ReliefComponent toCityGML(AbstractReliefComponentType reliefComponent, QName name) {		
	...
	public JAXBElement<? extends ReliefFeatureType> toJAXB(ReliefFeature reliefFeature) {
		if (!(reliefFeature instanceof ADEComponent))
			return ObjectFactory.DEM_1.createReliefFeature(((ReliefFeatureImpl)reliefFeature).getJAXBObject());
		else
	...
	public JAXBElement<? extends BreaklineReliefType> toJAXB(BreaklineRelief breaklineRelief) {
		if (!(breaklineRelief instanceof ADEComponent))
			return ObjectFactory.DEM_1.createBreaklineRelief(((BreaklineReliefImpl)breaklineRelief).getJAXBObject());
		else
	...
	public JAXBElement<? extends RasterReliefType> toJAXB(RasterRelief rasterRelief) {
		if (!(rasterRelief instanceof ADEComponent))
			return ObjectFactory.DEM_1.createRasterRelief(((RasterReliefImpl)rasterRelief).getJAXBObject());
		else
	...
	public JAXBElement<? extends MassPointReliefType> toJAXB(MassPointRelief massPointRelief) {
		if (!(massPointRelief instanceof ADEComponent))
			return ObjectFactory.DEM_1.createMassPointRelief(((MassPointReliefImpl)massPointRelief).getJAXBObject());
		else
	...
	public JAXBElement<? extends TINReliefType> toJAXB(TINRelief tinRelief) {
		if (!(tinRelief instanceof ADEComponent))
			return ObjectFactory.DEM_1.createTINRelief(((TINReliefImpl)tinRelief).getJAXBObject());
		else
			return ade2jaxb((ADEComponent)tinRelief, TINReliefType.class);
	}
	
	public JAXBElement<? extends AbstractReliefComponentType> toJAXB(ReliefComponent reliefComponent) {
		// we are just checking ADEs
		if (reliefComponent instanceof ADEComponent)
			return ade2jaxb((ADEComponent)reliefComponent, AbstractReliefComponentType.class);
	...
	public CityGMLBase toCityGML(JAXBElement<?> jaxbElem) {
		...
		if (jaxbElem.getValue() != null) {
			...
			else if (value instanceof AbstractReliefComponentType)
				cityGMLBase = toCityGML((AbstractReliefComponentType)value, name);
		}
