...
public class Transportation100ModelMapperImpl extends AbstractCityGMLModelMapperImpl {
	...
	public JAXBElement<? extends TrafficAreaType> toJAXB(TrafficArea trafficArea) {
		if (!(trafficArea instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createTrafficArea(((TrafficAreaImpl)trafficArea).getJAXBObject());
		else
			return ade2jaxb((ADEComponent)trafficArea, TrafficAreaType.class);
	}
	
	public TransportationObject toCityGML(AbstractTransportationObjectType transportationObject, QName name) {		
	...
	public JAXBElement<? extends AuxiliaryTrafficAreaType> toJAXB(AuxiliaryTrafficArea auxiliaryTrafficArea) {
		if (!(auxiliaryTrafficArea instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createAuxiliaryTrafficArea(((AuxiliaryTrafficAreaImpl)auxiliaryTrafficArea).getJAXBObject());
		else
	...
	public JAXBElement<? extends RailwayType> toJAXB(Railway railway) {
		if (!(railway instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createRailway(((RailwayImpl)railway).getJAXBObject());
		else
	...
	public JAXBElement<? extends RoadType> toJAXB(Road road) {
		if (!(road instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createRoad(((RoadImpl)road).getJAXBObject());
		else
	...
	public JAXBElement<? extends SquareType> toJAXB(Square square) {
		if (!(square instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createSquare(((SquareImpl)square).getJAXBObject());
		else
	...
	public JAXBElement<? extends TrackType> toJAXB(Track track) {
		if (!(track instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createTrack(((TrackImpl)track).getJAXBObject());
		else
	...
	public JAXBElement<? extends TransportationComplexType> toJAXB(TransportationComplex transportationComplex) {
		if (!(transportationComplex instanceof ADEComponent))
			return ObjectFactory.TRAN_1.createTransportationComplex(((TransportationComplexImpl)transportationComplex).getJAXBObject());
		else
			return ade2jaxb((ADEComponent)transportationComplex, TransportationComplexType.class);
	}
	
	public JAXBElement<? extends AbstractTransportationObjectType> toJAXB(TransportationObject transportationObject) {
		// we are just checking ADEs
		if (transportationObject instanceof ADEComponent)
			return ade2jaxb((ADEComponent)transportationObject, AbstractTransportationObjectType.class);
	...
	public CityGMLBase toCityGML(JAXBElement<?> jaxbElem) {
		...
		if (jaxbElem.getValue() != null) {
			...
			else if (value instanceof AbstractTransportationObjectType)
				cityGMLBase = toCityGML((AbstractTransportationObjectType)value, name);
		}
